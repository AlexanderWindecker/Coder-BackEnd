...
socketClient.on("list", (arrayPrdct) => {
  const listRender = arrayPrdct
    .map((elm) => {
      return `<ul class="ulContainer"><li class="title liContainer">${
        elm.title
      }</li>
        <li class="liContainer imgContainer"><img src=${
          elm.thumbnail
        } class="img"></li>
        ...
        <button class="buttonContainer" id="${
          elm._id
        }">Agregar Producto</button></ul>`;
    })
    .join(" ");
